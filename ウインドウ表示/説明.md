Windows APIを使ってアプリケーションウィンドウを作り、メッセージループでユーザー操作(閉じる等)を受け取り、ウィンドウが閉じられたらアプリケーションを終了する。

大まかな流れ：

1. `WinMain` が `new Window` で `Window` オブジェクトを生成する。
2. `Window` のコンストラクタ(`SetWindow`)内でウィンドウクラスを登録(`RegisterClassEx`)し、ウィンドウを作成(`CreateWindow`)する。
3. `WinMain` で(`ShowWindow`)を呼んでウィンドウを表示し、メッセージループ(`PeekMessage`)でメッセージ処理を行う。
4. ウィンドウが閉じられると`WM_DESTROY` が来て `PostQuitMessage(0)` → メッセージループが `WM_QUIT` を受け取って抜ける。
5. `delete win` により`Window` のデストラクタが呼ばれ、ウィンドウクラスの登録解除(`UnregisterClass`)を行い終了。
...
```
Windowsアプリケーションのメイン関数。\
・`_In_ HINSTANCE hInstance` アプリケーションインスタンスのハンドル。\
・`_In_opt_ HINSTANCE hPrevInstance` 前回実行したインスタンスのハンドル。\
・`_In_ LPSTR lpCmdLine` コマンドライン引数。\
・`_In_ int nCmdShow` ウインドウの初期状態。
...
```
ウインドウクラスのオブジェクトを作成。`MSG msg` はOS側から届くメッセージを格納する構造体。`ShowWindow` でウインドウを表示。
...
```
届いたメッセージが`WM_QUIT` なら終了。(`PostQuitMessage(0)` を呼ぶと`WM_QUIT` になる) `PeekMessage` でメッセージキューにメッセージがあるかを確認。`TranslateMessage` で文字メッセージに変換し、`DispatchMessage` でウインドウプロシージャに渡して処理させる流れ。
